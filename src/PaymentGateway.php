...
class PaymentGateway extends \WC_Payment_Gateway
{
	...
	public function __construct() {
		...
		// 支付网关标题
		$this->icon = apply_filters( 'hpj_Alipay_icon', XH_Alipay_Payment_URL . '/images/alipay-small.png' );
		...
		// 添加 URL
		add_action( 'woocommerce_api_hpj-wc-alipay-query', [ $this, 'query_order' ] );
		add_action( 'woocommerce_api_hpj-wc-alipay-notify', [ $this, 'listen_notify' ] );
	}
	...
	public function admin_options() { ?>
        ...
        <h3>
			<?php echo ( ! empty( $this->method_title ) ) ? $this->method_title : __( 'Settings', XH_Alipay_Payment ); ?>
	...
	function receipt_page( int $order_id ) {
	    $order = wc_get_order( $order_id );
        $turl = $this->get_option('transaction_url');
        ...
        $data=array(
              ...
              'time'      => time(),
              'notify_url'=> WC()->api_request_url( 'hpj-wc-alipay-notify' ),
            ...
            <div class="pay-weixin-design">
                 ...
                 <div class="p-w-center">
                    <h3>
            		   ...
            		   <strong>
            		      <img id="img1" src="<?php print XH_Alipay_Payment_URL?>/images/alipay-small.png">
            			  <span>支付宝</span>
            			  <img id="img2" src="<?php print XH_Alipay_Payment_URL?>/images/ep_new_sprites1.png">
            		   </strong>
            		</h3>
            	    <h4>通过支付宝扫描二维码进行支付，本页面将在支付完成后自动刷新。</h4>
            	 ...
            	 <div class="p-w-right">
            	    <img src="<?php print XH_Alipay_Payment_URL?>/images/alipay-sys.png">
            	 </div>
        ...
        <script type="text/javascript">
        		(function () {
        		    function queryOrderStatus() {
        		        ...
        		        $.ajax({
        		            type: "GET",
        		            url: '<?php print WC()->api_request_url( 'hpj-wc-alipay-query' );?>',
        		            data: {
	...
	public function query_order() {
        ...
        if ( $order_id ) {
            ...
            $url = $this->get_option('transaction_url').'/payment/query.html';
            ...
            if($result['data']['status']=='OD'){
                ...
                if(!(method_exists($order, 'is_paid')?$order->is_paid():in_array($order->get_status(),  array( 'processing', 'completed' )))){
                    ...
                    WC()->cart->empty_cart();
                    // 添加订单备注
                    $order->add_order_note(sprintf( __( 'Alipay payment complete (Transaction ID: %s)', XH_Alipay_Payment ), $result['data']['transaction_id']));
                }
	...
	public function listen_notify() {
        ...
        if(!(method_exists($order, 'is_paid')?$order->is_paid():in_array($order->get_status(),  array( 'processing', 'completed' )))&&$data['status']=='OD'){
            ...
            WC()->cart->empty_cart();
            // 添加订单备注
            $order->add_order_note(sprintf( __( 'Alipay payment complete (Transaction ID: %s)', XH_Alipay_Payment ), $data[ 'transaction_id' ] ));
        }
        print 'success';
        exit;
	}
	...
	public function log( $message ) {
		if ( $this->is_debug_mod ) {
			...
			$this->log->add( 'xh-alipay-payment-wc', var_export( $message, true ) );
		}
	}
}